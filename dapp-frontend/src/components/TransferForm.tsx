...
export default function TransferForm({ contract, address }: { contract: any; address: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2 w-full bg-gray-50 dark:bg-gray-900 p-4 rounded shadow">
      ...
      <input type="text" placeholder="Endereço de destino" value={to} onChange={e => setTo(e.target.value)} className="px-3 py-2 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-black" required />
      ...
      <button type="submit" disabled={loading} className="mt-2 px-4 py-2 rounded bg-black text-white dark:bg-white dark:text-black border border-black dark:border-white hover:opacity-80 transition disabled:opacity-50">{loading ? "Enviando..." : "Transferir"}</button>
